...
class BigqueryService {
  ...
  queryCalls = async ({ agent_id, limit = 50 }: CallsQuery): Promise<CallTranscript[]> => {
    if (bigqueryClient && DATASET) {
      ...
      const sql = `SELECT * FROM \`${env.GCP_PROJECT_ID}.${DATASET}.call_transcripts\` ORDER BY created_at DESC LIMIT @limit`;
  ...
  getCallById = async (call_id: string): Promise<CallTranscript | undefined> => {
    if (bigqueryClient && DATASET) {
      const sql = `SELECT * FROM \`${env.GCP_PROJECT_ID}.${DATASET}.call_transcripts\` WHERE call_id='${call_id}' LIMIT 1`;
  ...
  getAnalysisByCallId = async (call_id: string): Promise<AnalyzedCall | undefined> => {
    if (bigqueryClient && DATASET) {
      const sql = `SELECT * FROM \`${env.GCP_PROJECT_ID}.${DATASET}.analysed_calls\` WHERE call_id='${call_id}' LIMIT 1`;
      const [job] = await bigqueryClient.createQueryJob({ query: sql });
      const [rows] = await job.getQueryResults();
